    ...
    while True:
        ...
        if menu == "q":
            ...
            if levels == 1:
                while True:
                    random_simple_color()
                    print("What is the translation of this color:", simples_color_english)
                    response = str(input("> "))
                ...
                while True:
                    ...
                    print("What is the translation of this color:", advanced_color_english)
                    response = str(input("> "))
                    if response.lower().strip() == advanced_color_portuguese.lower():
                        print("Correct! +1 point.")
                        points += 1
                        print("Current points:", points)
                        if not ask_play_again():
                ...
                while True:
                        random_simple_color(),random_advanced_color()
                        print("What is the translation of this color:", simples_color_english, advanced_color_english)
                        response_simple = str(input("first color> "))
                        response_advanced = str(input("second color> "))
                        if response_simple.lower() == simple_color_portuguese.lower():
                            if response_advanced.lower() == advanced_color_portuguese.lower():
                                print("Correct! +1 point.")
                                points += 1
                                print("Current points:", points)
                                if  points == 30:
                        ...
                        if response_simple.lower() != simple_color_portuguese.lower() or response_advanced.lower() != advanced_color_portuguese.lower():
                            points -= 1
                            print('Incorrect answer!')
                            print(f'Correct answer: {simple_color_portuguese}, {advanced_color_portuguese}')
